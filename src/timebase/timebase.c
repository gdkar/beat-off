...
static double freq_mb_per_s = 140. / 60;
static long pt_ms = 0;
static long pt_mb = 0;
...
{
    return pt_mb + (long)((double)(cur_ms - pt_ms) * freq_mb_per_s);
}
...
void timebase_tap(double alpha)
{
    ...
    freq_mb_per_s = alpha * new_freq + (1. - alpha) * freq_mb_per_s;

    pt_ms = cur_ms;
    pt_mb = cur_mb;

    ui_bpm = ui_bpm * config.timebase.ui_bpm_alpha + (1.0 - config.timebase.ui_bpm_alpha) * (freq_mb_per_s * 60.);
