...
void mouse_drag_alpha_slider(struct xy xy){
    float val = active_slider.initial_value - (float)xy.y / (layout.alpha_slider.track_h - layout.alpha_slider.handle_h);
    if(val < 0) val = 0;
    else if(val > 1) val = 1;
    param_state_setq(active_slider.state, val);
}
void mouse_drag_param_slider(struct xy xy){
    float val = active_slider.initial_value + (float)xy.x / (layout.slider.track_w - layout.slider.handle_w);
    if(val < 0) val = 0;
    else if(val > 1) val = 1;
    param_state_setq(active_slider.state, val);
}
