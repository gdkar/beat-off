...
void ui_init(){
    if (SDL_Init(SDL_INIT_VIDEO)){FAIL("SDL_Init Error: %s\n", SDL_GetError());}
...
static inline int x_to_px(float x){return (int)(((x + 1) / 2) * layout.master.w);}
static inline int y_to_px(float y){return (int)(((y + 1) / 2) * layout.master.h);}
static inline int SDL_line(SDL_Surface* dst, int16_t x1, int16_t y1, int16_t x2, int16_t y2,
...
static void ui_update_midi(struct midi_controller * controller){
    ...
    if(controller->available) color = controller->color;
...
static void mouse_drag_pattern(struct xy xy){active_pattern.dxy = xy; }
static void mouse_drag_hit(struct xy xy){active_hit.dxy = xy; }
static void mouse_drag_slot(struct xy xy){active_slot.dxy = xy; }
...
static void mouse_drop_slot(){
    ...
    if(active_slot.is_pattern){
        ...
        if(xy.x > max_x){pat_unload(active_slot.slot);}
    }else{
        for(int i = 0; i < n_hit_slots; i++){
            rect_array_layout(&layout.hit_slot.rect_array, i, &r);
            if(in_rect(&xy, &r, 0)){
        ...
        if(xy.x > max_x){hit_unload(active_slot.slot);}
    }
    active_slot.slot = 0;
}
...
static int mouse_click_audio(struct xy xy){
    ...
    if(in_rect(&xy, &layout.waveform.rect, &offset)){timebase_tap();}
    return 1;
}
static int mouse_click(struct xy xy){
    ...
    if(in_rect(&xy, &layout.master.rect, &offset)){return 0;}
    // See if click is in audio pane
    if(in_rect(&xy, &layout.audio.rect, &offset)){return mouse_click_audio(offset);}
    ...
    for(int i = 0; i < n_signals; i++){
        rect_array_layout(&layout.signal.rect_array, i, &r);
        if(in_rect(&xy, &r, &offset)){return mouse_click_signal(i, offset);}
    }
    // See if click is in filter
    for(int i = 0; i < n_filters; i++){
        rect_array_layout(&layout.filter.rect_array, i, &r);
        if(in_rect(&xy, &r, &offset)){return mouse_click_filter(i, offset);}
    }
    // See if click is in output
    for(int i =  0; i < n_output_strips; i++){
        rect_array_layout(&layout.output.rect_array, i, &r);
        if(in_rect(&xy, &r, &offset)){return mouse_click_output(i, offset);}
    }
    // See if click is in midi
    for(int i =  0; i < n_controllers_enabled; i++){
        rect_array_layout(&layout.midi.rect_array, i, &r);
        if(in_rect(&xy, &r, &offset)){return mouse_click_midi(i, offset);}
    }
